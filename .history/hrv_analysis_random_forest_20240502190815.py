...
def evaluate_model(model, X_train, X_test, y_train, y_test):
    ...
    train_f1 = f1_score(y_train, y_train_pred, average='macro')  # 'macro' はクラスバランスを考慮しない
    ...
    sns.heatmap(cm_train, annot=True, fmt='d', cmap='Blues', xticklabels=[1, 2, 3, 4, 5], yticklabels=[1, 2, 3, 4, 5])
    ...
    plt.figure(figsize=(10,7))
    sns.heatmap(cm_test, annot=True, fmt='d', cmap='Blues', xticklabels=[1, 2, 3, 4, 5], yticklabels=[1, 2, 3, 4, 5])
    plt.xlabel('Predicted')
    plt.ylabel('True')
...
# y が 3 でないデータのインデックスを取得
indices = y != 3

# y から y が 3 のデータを取り除く
y = y[indices]

# X から同じく y が 3 のデータに対応する行を取り除く
X = X[indices]

# 1と2を0に、4と5を1に変換
y = np.where(np.isin(y, [1, 2]), 0, np.where(np.isin(y, [4, 5]), 1, y))
