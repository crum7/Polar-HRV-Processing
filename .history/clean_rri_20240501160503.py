...
for rri in rri_list:
    ...
    cleaned_rri = preprocessing.remove_outliers(rr_intervals=raw_rri,verbose=True) 
    fill_nan_rri = preprocessing.interpolate_nan_values(rr_intervals=cleaned_rri,interpolation_method="linear")
    nn_intervals_list = remove_ectopic_beats(rr_intervals=interpolated_rr_intervals, method="malik")
    # This replace ectopic beats nan values with linear interpolation
    interpolated_nn_intervals = interpolate_nan_values(rr_intervals=nn_intervals_list)
    preprocessed_rri_list.append(fill_nan_rri)
